...
use app\models\AxionValuationSearch;
use app\models\User;
...
use app\models\AxionValuationHistory;
use app\models\PreinspectionClientCaller;
use app\models\EmailHistory;
use app\models\AxionValuationFourwheeler;
use app\models\AxionValuationCommercial;
use app\models\AxionValuationPhotos;
 ...
 */
class AxionValuationController extends Controller
{
    ...
    public function actionIndex()
    {
         ...
         */ 
               ...
               // your default model and dataProvider generated by gii
                $searchModel = new AxionValuationSearch();
                ...
                if(isset($params['AxionValuationSearch']['insurerBranch']))
                {
                    
                    $branch = PreinspectionClientBranch::find()
                        ->where(['companyId' => $params['AxionValuationSearch']['insurerName']])
                        ->all();  
    ...
    public function actionCompleted()
    {
        ...
        $role = array_keys(Yii::$app->authManager->getRolesByUser(Yii::$app->user->getId()))[0] ;
               ...
               // your default model and dataProvider generated by gii
                $searchModel = new AxionValuationSearch();
                $searchModel->status = [8,101,102,104,200]; 
    ...
    public function actionTransaction($id)
    {
        ...
        $query = AxionValuationHistory::find()->where(['preinspection_id' => $id]);
   ...
   protected function saveImageObj($obj,$id,$lastImage) {
        ...
        $image =  UploadedFile::getInstance($obj,'image['.$obj->type.']');
            ...
            $imgData = $this->get_image_location($inputFile);
                    ...
                    if($lastImage != '')
                    {
                        $filename = getcwd().'/'.$vqcLoc.$lastImage;
      ...
      public function actionImageUpload()
    {
        ...
        $id = $_POST['id'];
        $type = $_POST['type'];
        $obj = AxionValuationPhotos::findOne(['preinspection_id' => $id,'type' => $type]);
    ...
    public function actionVideoSession($id)
    {
        ...
        $premodel = AxionValuation::findOne($id);
    ...
    public function actionFourwheelerqc($id)
    {
       ...
       $premodel = AxionValuation::findOne($id);
        ...
        // $first = (int)$premodel->syd1to5/12;
        // $second = (int)$premodel->syd6to8/12;

       $cmodel = VehicleMake::find()
       ->where(['id' => $premodel->makeId])
       ->all();

       $vmodel = VehicleModel::find()
       ->where(['id' => $premodel->modelId])
       ->all(); 

       $exmodel = VehicleVariant::find()
       ->where(['id' => $premodel->variantId])
       ->all(); 

    // $fourth = $premodel->syd1to5;
// find age  2
            
        $date1 = date("Y");
        $date2 = date('m')-1;
        $month = $premodel->month;
        $premodel->age = ((($date1 - $premodel->manufacturingYear)*12)+$date2)-($premodel->month);
 ...
 // set syd condition  3         
        ...
        if ($premodel->age > "60") 
        {
          $premodel->syd1to5 = 10;
          $premodel->syd6to8 = 5;
          $first = $premodel->syd1to5/60;
          $div = $premodel->age - 60;
          $second = $premodel->syd6to8/$div;
          $third = $first + $second;
          (int)$premodel->calculation = $third;
        }
        
        else
        {
          // $fourth = 10/12;
          (int)$premodel->calculation = 10/$premodel->age;
        }
// $premodel->save();

// calculate depri  4        
         $premodel->currentDepric = $premodel->age * $premodel->calculation;
         $premodel->save();

    
         $completedSurveyDateTime = $premodel->completedSurveyDateTime;
         $customerAppointDateTime = $premodel->customerAppointDateTime;
         $registrationYear = $premodel->registrationYear;
         $phmodel = AxionValuationPhotos::findAll(['preinspection_id' => $id]);
        if(count($phmodel) == 0)
        {
         ...
         $model = AxionValuationFourwheeler::findOne(['preinspection_id' => $id]);
        // print_r($model);
       // $premodel->currentDepric =$model->finalRating + $premodel->currentDepric;
        ...
        // $make = VehicleMake::find()
        //             ->where(['id' => $make->make_name])
        //             ->all();        
        
         $customerSession = User::find()
        ->where(['mobile' => $premodel->customerMobile])
        ->count();
        // print_r($model);
...
// Add rating     5       
         ...
         $fbb = $model->frontBumperBonnet;
         $pc =  $model->paintCondition;
         $rfd = $model->rhFenderDoors;
         $lfd = $model->lhFenderDoors;
         $cc = $model->chassisCondition;
         $rbb = $model->rearBumperBonnet;
         $trc = $model->transmissionCondition;
         $ac = $model->airCondition;
         $ia = $model->interierAccessories;
         $cvf = $model->chassisAndVehicleFrame;
         $br = $model->brakes;
         $sp = $model->suspension;
         $bc = $model->batteryCondition;
         $tc = $model->tyreCondition;
         $eng = $model->engineCondition;
         $sc = $model->seatsCondition;
         $vc = $model->vehicleCondition;
         $hltl = $model->headLampAndTailLamp;
        
         $fr = (int)$fbb+(int)$pc+(int)$rfd+(int)$lfd+(int)$cc+(int)$rbb+(int)$trc+(int)$ac+(int)$ia+(int)$cvf+(int)$br+(int)$br+(int)$sp+(int)$bc+(int)$tc+(int)$eng+(int)$sc+(int)$vc+(int)$hltl;
...
// depri calculate 6
         $premodel->depriAmount = ((int)$premodel->exShowroomPrice * (int)$premodel->currentDepric /100);
// price calculate    7         
         $premodel->systemGeneratedMarketValue =((int)$premodel->exShowroomPrice - (int)$premodel->depriAmount);
// edit final calculation   8          
         $fValue  = $premodel->systemGeneratedMarketValue;

         if($premodel->fixedMarketValue == '' )
         {
            $premodel->fixedMarketValue = $fValue;
         } 
...
// find age  2
 ...
 // set syd condition  3         
        ...
        {
        $fourth = 10/12;
        (int)$premodel->calculation = $fourth;
        }
// $premodel->save();
...
// calculate depri  4        
    ...
    $premodel->save();
        ...
        $model = AxionValuationCommercial::findOne(['preinspection_id' => $id]);
...
//          // $model = AxionPreinspectionFourwheeler::findOne(['preinspection_id' => $id]);
         ...
         $it = $model->interior;
         $fcb =  $model->frontCabin;
         $rr = $model->rear;
         $cdr = $model->cabinDoorRight;
         $cdl = $model->cabinDoorLeft;
         $trc = $model->transmissionCondition;
         $olb = $model->overAllLoadBody;
         $lbc = $model->leftBodyCondition;
         $rbc = $model->rightBodyCondition;
         $clc = $model->cluthCondition;
         $hltl = $model->headLampTailLamp;
         $br = $model->brakes;
         $dc = $model->differentialCondition;
         $ch = $model->chassisCondition;
         $paint = $model->paintCondition;
         $sus = $model->suspension;
         $seat = $model->seatsCondition;
         $axc = $model->axleCondition;

         $lf = $model->loadFloor;
         $tg = $model->tailGate;
         $tyre = $model->tyreCondition;
         $batry = $model->batteryCondition;
         $eng = $model->engineCondition;

        
    $fr = (int)$it+(int)$fcb+(int)$rr+(int)$cdr+(int)$cdl+(int)$trc+(int)$olb+(int)$lbc+(int)$rbc+(int)$clc+(int)$hltl+(int)$br+(int)$dc+(int)$ch+(int)$paint+(int)$sus+(int)$seat+(int)$axc+(int)$lf+(int)$tg+(int)$tyre+(int)$batry+(int)$eng;
         
//          // die('test');
    $model->finalRating = $fr;
...
// print_r($model->getErrors());
    ...
    {
       $model = AxionValuation::findOne($id);       
    ...
    public function actionValuatorall($id)
    {
        if($id == '')
        {
    ...
    protected function updateFieldexecutivesTask($processModel,$updateType)
    {
        ...
        if($updateType == 'insert')
        {
            ...
            $model->intimationDate = $processModel->intimationDate;
    ...
    {
        $phmodel = new AxionValuationPhotos();
    ...
    protected function updateQc($preModel,$updateType)
    {
        if($preModel->vehicleType == '4-WHEELER')
        {
            if($updateType == 'insert')
            {
                $countPosts = AxionValuationFourwheeler::find()
                ...
                if($countPosts == 0)
                {
                    $model = new AxionValuationFourwheeler();
             ...
             // $fValue  = $premodel->systemGeneratedMarketValue; 
            // $fValue = $premodel->fixedMarketValue;
             // $premodel->save();
            ...
            if($updateType == 'delete')
            {
                 AxionValuationFourwheeler::deleteAll(['preinspection_id' => $preModel->id]);
                 AxionValuationPhotos::deleteAll(['preinspection_id' => $preModel->id]);
            }
        }
        if($preModel->vehicleType == 'COMMERCIAL')
        {
            if($updateType == 'insert')
            {
                $countPosts = AxionValuationCommercial::find()
                        ->where(['preinspection_id' => $preModel->id])
                        ->count();

                if($countPosts == 0)
                {
                    $model = new AxionValuationCommercial();
                    $model->preinspection_id = $preModel->id;
                    $model->save();
            ...
            if($updateType == 'delete')
            {
                AxionValuationCommercial::deleteAll(['preinspection_id' => $preModel->id]);
                AxionValuationPhotos::deleteAll(['preinspection_id' => $preModel->id]);
            }
        }
    ...
    protected function createDropboxFolder($fileDate,$surveyorName,$vehicleNumber)
    {
        $dropboxImageLoc = \Yii::$app->params['dropboxImageLoc'];
        $fileDate = date('d-m-Y',strtotime($fileDate));
        $path = $dropboxImageLoc.'/PI/'.$fileDate.'/'.$surveyorName.'/'.$vehicleNumber;
        FileHelper::createDirectory($path);
    }
    ...
    protected function createCustomerSession($id)
    {
        ...
        $model = AxionValuation::findOne($id);
        $countPosts = User::find()
        ->andFilterWhere(['mobile' => $model->customerMobile])
        ->count();

        if($countPosts>0){
            $delUser = User::findOne(['mobile' => $model->customerMobile]);
            if($delUser)
            {
                $auth = new DbManager;
                $auth->init();
       ...
       $cmodel->email = $model->customerMobile.'@pcs.in';
       $cmodel->password = $model->customerMobile;
       $cmodel->password_repeat = $model->customerMobile;
       $cmodel->mobile = $model->customerMobile;
       $cmodel->firstName = $model->customerName;
       ...
       if($cmodel->save())
       {
            ...
            if($model->ro == '')
            {
               ...
               {
                    $link = Yii::$app->urlManager->createAbsoluteUrl('axion-valuation/fourwheelerqc?id='.$model->id);
               }
               else
               {
                   $link = Yii::$app->urlManager->createAbsoluteUrl('axion-valuation/commercialqc?id='.$model->id);
               }

                $mobileno = $model->customerMobile;
                $message = 'Username:'.$model->customerMobile.'@pcs.in Password:'.$model->customerMobile.' Quality Check Url - '.$link;
                ...
                $emailMessage = 'Username:'.$model->customerMobile.'@pcs.in Password:'.$model->customerMobile.' Quality Check Url - '.$link;
    ...
    public function actionCreate()
    {
        ...
        $vmodel = ['id' => '', 'model' => 'Select'];
        $exmodel = ['id' => '', 'variant' => 'select'];
        $exShowroomPrice = ['id' => '','ex_showroom_price' => 'Select'];
        ...
        if (Yii::$app->request->isAjax && $model->load(Yii::$app->request->post())) {
                ...
                {
                    $model->cashToBeCollected = date('Y-m-d H:i:s');
                }

                 if($model->save())
                 {
                    if($model->surveyorName != '' && $model->surveyorName == 0 && $model->customerMobile != '')
                    {
                    ...
                    $hismodel = new AxionValuationHistory();
    ...
    {
        if (($model = AxionValuation::find()->orderBy(['id' => SORT_DESC])->one()) !== null) {
    ...
    protected function hisMessage($id)
    {
        ...
        $his = AxionValuationHistory::find()
    ...
    public function actionUpdate($id)
    {
        ...
        $lastrequestDateTime = $model->intimationDate;
       ...
       if (Yii::$app->request->isAjax && $model->load(Yii::$app->request->post())) {
                ...
                if($model->surveyorName != 0)
                {
                    ...
                    $surveyor_name =  $valuatorModel->firstName;
                    $valuator_mobile =  $valuatorModel->mobile;
                }
                if($model->surveyorName == 0)
                {    
                    $surveyor_name =  'Customer - '.$model->customerName;
                    $valuator_mobile =  $model->customerMobile;
                }
                
               if($lastsurveyorName != $model->surveyorName)
               {
                   $model->cashToBeCollected = $currentDateTime;
               }
               else if($model->surveyorName != '')
               {
                    $sadate = \DateTime::createFromFormat('d/m/Y h:i a', $model->cashToBeCollected );
                    if($sadate)
                    {
                       $model->cashToBeCollected = $sadate->format('Y-m-d H:i:s');
                    }
               }

                $model->intimationDate = $lastrequestDateTime;
                $model->userId = Yii::$app->user->identity->id;
                ...
                if($lastStatus == 0 && $model->status == 0 && $model->followupReason != 0 && $model->followupRemainder != '' && $model->followupUpdatedBy !='')
                {
                    ...
                    {
                        $model->followupUpdatedDateTime = date('Y-m-d H:i:s');
                    }
                    else
                    {
                    ...
                    if($model->save())
                    {
                        ...
                        if($model->status == 12)
                        {
                            ...
                            if($_POST['sendSms'] == 'Y')
                            {
                                ...
                                $message = 'STATUS- '.$model->registrationNo.'/'.$model->referenceNo.' SCHEDULE APP - '.date( 'd/m/Y h:i A', strtotime( $model->customerAppointDateTime )).' VALUATOR NAME:('.$surveyor_name.') '.$additionalSmsMessage;
                                $this->sendSms($mobileno, $message); 
                                
                                //sending sms to surveyor
                                $surveyor_message = $model->referenceNo.' '.$model->customerName.' '.$model->contactPersonMobileNo.' '.$model->vehicleLocation.' '.date( 'd/m/Y h:i A', strtotime( $model->customerAppointDateTime ));
                        ...
                        else if($model->status == 1)
                        {
                            ...
                            {
                                $mobileno = $model->callerMobileNo;
                                $message = 'STATUS- '.$model->registrationNo.'/'.$model->referenceNo.' RE - SCHEDULE APP - '.date( 'd/m/Y h:i A', strtotime( $model->customerAppointDateTime )).' REASON:'.'('.$model->rescheduleReason.') VALUATOR NAME:('.$surveyor_name.') '.$additionalSmsMessage;
                                $this->sendSms($mobileno, $message);
                        ...
                        if($lastStatus == 0 && $model->status == 0 && $model->followupReason != 0 && $model->followupRemainder != '' && $model->followupUpdatedBy !='')
                        {
                            ...
                            if($lastFollowupReason != $model->followupReason || $lastFollowupRemainder != $model->followupRemainder || $lastFollowupUpdatedBy != $model->followupUpdatedBy)
                            {
                                    ...
                                    if($model->followupReason == 1 || $model->followupReason == 2 || $model->followupReason == 3 || $model->followupReason == 4)
                                    {
                                        ...
                                        //insured mobile no
                                        $customerMobileno = $model->insuredMobile;
                                        if($customerMobileno != '')
                                        {
    ...
    public function actionDelete($id)
    {
		...
		*/
        ...
        if (!$id) {
          ...
          AxionValuation::findOne($id)->delete();
    ...
    public function actionDeleteMultiple()
    {
        ...
        {
          AxionValuation::findOne($value)->delete();
        }
        return;
    }
    ...
    public function actionRemovePhoto()
    {
        ...
        $vqcLoc = \Yii::$app->params['vqcLoc'];
            ...
            $model = AxionValuationPhotos::findOne($id);
            $filename = getcwd().'/'.$vqcLoc.$model->image;
            if(unlink($filename))
            {
                $model->image = '';
                $model->iLat = '';
    ...
    public function actionDownloadphotos($id)
    {
        ...
        $tmp_file = getcwd().'/upload/'.$referenceNo.'-images.zip';
        ...
        if(count($phmodel) > 0)
        {
             ...
             $zip->close();
                ...
                $path = getcwd().'/upload/'.$referenceNo.'-images.zip';
                if(file_exists($path)){
    ...
    {
        if (($model = AxionValuation::findOne($id)) !== null) {
            return $model;
        } else {
    ...
    public function actionVehicleCheck($regno)
    {
            ...
            $countPosts = AxionValuation::find()
